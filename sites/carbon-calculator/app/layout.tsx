      ...
      <body className={`${inter.variable} font-sans antialiased bg-stone-50 text-stone-900`}>
        <header className="border-b border-stone-200 bg-white/80 backdrop-blur-sm sticky top-0 z-50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
            ...
            <nav className="flex items-center gap-3 sm:gap-6 text-sm">
              <Link
                href="/guide/remote-work-carbon-footprint"
                className="text-stone-600 hover:text-stone-900 hidden sm:inline"
              >
                指南
              </Link>
              <Link href="/calculate" className="text-stone-600 hover:text-stone-900 hidden sm:inline">
                计算
              </Link>
              <Link
                href="/join"
        ...
        <footer className="border-t border-stone-200 bg-white mt-16">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 text-center text-sm text-stone-500">
            <p>© {new Date().getFullYear()} 远程碳足迹 · 混合办公碳排放计算器</p>
            <p className="mt-2">
              <Link href="/guide/remote-work-carbon-footprint" className="text-brand-600 hover:underline">
                远程办公碳足迹完整指南
              </Link>
              {" · "}

...
import { createWorkspace, listWorkspaces, syncWorkspace } from "@/lib/health";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const workspace = createWorkspace(body.name.trim(), body.stripeKey);
    return NextResponse.json({ workspace });
  } catch (error) {
    console.error("Health sync error:", error);
    return apiError("HEALTH_SYNC_FAILED", 500);
  }
}

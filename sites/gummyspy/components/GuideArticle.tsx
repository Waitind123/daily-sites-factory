    ...
    <article className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16 prose prose-stone prose-invert">
      ...
      {g.sections.map((section) => (
        <div key={section.h2}>
              ...
              <div key={tool.h3}>
                ...
                <p>
                  ...
                  {"link" in tool && tool.link && (
                    <>
                      {" "}
                      <Link href={tool.link.href} className="text-brand-500 hover:underline">
                        {tool.link.text}
                      </Link>
                      {tool.link.suffix}
                    </>
          ...
          {"afterLink" in section && section.afterLink && (
            <p>
              {section.afterLink.prefix}
              <Link href={section.afterLink.href} className="text-brand-500 hover:underline">
                {section.afterLink.label}
              </Link>
              {section.afterLink.suffix}
            </p>
          )}
          {"closingLink" in section && section.closingLink && (
            <p>
              {section.closingLink.prefix}
              <Link href={section.closingLink.href} className="text-brand-500 hover:underline">
                {section.closingLink.label}
              </Link>
              {section.closingLink.suffix}
            </p>
          )}
        </div>
      ))}

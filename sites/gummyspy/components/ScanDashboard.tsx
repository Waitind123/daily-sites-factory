...
import type { PainPoint, PainPlatform } from "@/lib/pain-miner";
...
const PLATFORM_COLORS: Record<PainPlatform, string> = {
  reddit: "bg-orange-500/20 text-orange-400",
  hn: "bg-amber-500/20 text-amber-400",
  producthunt: "bg-rose-500/20 text-rose-400",
};

export function ScanDashboard({ locale }: { locale: Locale }) {
  const t = getScanCopy(locale);
  ...
  async function handleScan(e: React.FormEvent) {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : getApiErrorMessage("SCAN_FAILED", locale)
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
      <div className="mb-8">
        ...
        <Link href="/spy" className="mt-2 inline-block text-sm text-brand-400 hover:text-brand-300">
          {t.spyLink}
        </Link>
      </div>
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-wrap gap-3 mb-4 text-sm">
            ...
            <span className="rounded-full bg-brand-600/20 text-brand-400 px-3 py-0.5 font-medium">
              {t.opportunityScore}: {result.avgOpportunityScore}/100
            </span>
            {(Object.entries(result.platforms) as [PainPlatform, number][])
              .filter(([, n]) => n > 0)
              .map(([platform, count]) => (
                <span
                  key={platform}
                  className={`rounded-full px-3 py-0.5 text-xs font-medium ${PLATFORM_COLORS[platform]}`}
                >
                  {getPlatformLabel(platform, locale)} ×{count}
                </span>
              ))}
          </div>
          <div className="space-y-4">
            {result.points.map((point) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded ${PLATFORM_COLORS[point.platform]}`}
                  >
                    {getPlatformLabel(point.platform, locale)}
                  </span>
                  <span className="text-xs text-muted">
                    {t.intensity}: {point.intensity}/10
                  </span>
                  <span className="text-xs text-muted">
                    {t.score}: {point.opportunityScore}/100
                  </span>
                  <span className="text-xs text-muted">
                    {t.source}: {point.source}
                  </span>
                </div>
              ...
              {sample.points.map((point, i) => (
                <div key={i} className="text-sm border-t border-border pt-3 first:border-0 first:pt-0">
                  <div className="flex gap-2 mb-1 flex-wrap">
                    ...
                    <span className="text-xs text-muted">
                      {t.intensity} {point.intensity}/10 · {point.source}
                    </span>
                  </div>

        ...
        <div className="mb-8 space-y-6">
          ...
          <div className="rounded-xl border border-border bg-surface p-6">
            ...
            <div className="space-y-3">
              {result.heatmap.map((zone) => (
                <div key={zone.section} className="flex items-center gap-3">
                  <span className="w-32 text-sm text-muted shrink-0">{zone.section}</span>
                  ...
                  <span className="text-xs text-muted w-16 text-right">{zone.attention}%</span>
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${
                      zone.dropOffRisk === "high"
                        ? "bg-red-500/20 text-red-400"
                        : zone.dropOffRisk === "medium"
                          ? "bg-amber-500/20 text-amber-400"
                          : "bg-emerald-500/20 text-emerald-400"
                    }`}
                  >
                    {riskLabel(zone.dropOffRisk)}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-xl border border-border bg-surface p-6">
            <h3 className="font-semibold text-foreground mb-4">{t.dropOffTitle}</h3>
            <div className="space-y-3">
              {result.dropOffPoints.map((point) => (
                <div key={point.step} className="rounded-lg border border-border bg-background p-4">
                  <div className="flex justify-between items-start gap-2">
                    <p className="font-medium text-foreground text-sm">{point.step}</p>
                    <span className="text-red-400 text-sm font-semibold shrink-0">{point.rate}</span>
                  </div>
                  <p className="text-sm text-muted mt-2">{point.fix}</p>
                </div>
              ))}
            </div>
          </div>

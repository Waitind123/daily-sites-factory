...
import { getLegalCopy, termsOfServiceBody } from "@/lib/legal";
...
export default async function TermsPage() {
  ...
  const body = termsOfServiceBody(locale, name);
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-12 sm:px-6 prose prose-invert prose-sm sm:prose-base">
      ...
      <h1 className="text-3xl font-bold text-foreground not-prose">{legal.termsTitle}</h1>
      <p className="text-muted not-prose mt-2">{legal.termsMeta}</p>

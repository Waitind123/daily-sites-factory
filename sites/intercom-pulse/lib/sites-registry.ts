...
export function loadSitesFromState(): SiteEntry[] {
  ...
  const factoryBundled = join(process.cwd(), "..", "factory-dashboard", "data", "sites.json");
  if (existsSync(factoryBundled)) {
    return JSON.parse(readFileSync(factoryBundled, "utf8")) as SiteEntry[];
  }

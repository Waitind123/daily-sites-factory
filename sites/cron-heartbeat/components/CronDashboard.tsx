...
export function CronDashboard({ locale }: { locale: Locale }) {
  ...
  function statusLabel(status: string) {
    if (status === "healthy") return t.statusHealthy;
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      <form onSubmit={handleCreate} className="rounded-xl border border-border bg-surface p-6 mb-8">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1">{t.graceLabel}</label>
            <input
              type="number"
              min={1}
              max={1440}
      ...
      {result && (
        <div className="rounded-xl border border-border bg-surface p-6 mb-8">
          ...
          <dl className="grid gap-3 text-sm mb-4">
            <div>
              <dt className="text-muted mb-1">{t.pingUrl}</dt>
            ...
            {sampleJobs.map((j) => (
              ...
              >
                ...
                <span className={`font-medium shrink-0 ml-2 ${statusColor(j.status)}`}>
                  {statusLabel(j.status)} · {j.lastPing}
                </span>
              </li>
            ))}
          </ul>

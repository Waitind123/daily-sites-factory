...
import { getApiErrorMessage, getWikiCopy } from "@/lib/copy-app";
...
export function WikiDashboard({ locale }: { locale: Locale }) {
  const t = getWikiCopy(locale);
  ...
  const [created, setCreated] = useState<WikiSpace | null>(null);
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setCreated(data.space);
      setSpaces((list) => [data.space, ...list]);
      setName("");
      setDescription("");
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      {created && (
        <div className="mb-8 rounded-xl border border-green-600/40 bg-green-600/10 p-6">
          ...
          <code className="block mt-2 text-sm font-mono break-all">/w/{created.slug}</code>
          <Link href={`/w/${created.slug}`} className="mt-3 inline-block text-brand-500 text-sm hover:underline">
            {t.openSite}
          </Link>
        </div>
      )}
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted mt-0.5">
                      {space.pages.length} {t.pages} · {space.viewCount} {t.views}
                    </p>
                  </div>

...
import type { Dashboard, MetricSnapshot, CohortRow } from "@/lib/metrics";
...
function CohortTable({ cohorts, locale }: { cohorts: CohortRow[]; locale: Locale }) {
  const t = getDashboardCopy(locale);
  return (
    <div className="mt-6 rounded-xl border border-border bg-background p-4">
      <h3 className="text-sm font-semibold text-foreground mb-3">{t.cohortTitle}</h3>
      <div className="space-y-3">
        {cohorts.map((c) => (
          <div key={c.month} className="flex items-center gap-3 text-sm">
            <span className="w-24 shrink-0 text-muted">{c.month}</span>
            ...
            <span className="w-12 text-right font-medium text-foreground">{c.retention}%</span>
            <span className="w-16 text-right text-xs text-muted">{c.signups} {locale === "zh" ? "人" : "users"}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="rounded-2xl border border-brand-600/30 bg-surface p-6 shadow-xl">
      ...
      {metrics.cohorts && metrics.cohorts.length > 0 && (
        <CohortTable cohorts={metrics.cohorts} locale={locale} />
      )}
    </div>
  );
}

  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-stone-200 bg-white shadow-sm">
      <table className="w-full text-sm">
        ...
        <tbody>
          {preview.map((venue) => (
            <tr key={venue.id} className="border-b border-stone-50 hover:bg-stone-50/50">
              <td className="px-4 py-3">
                <span className="mr-2">{venue.logo}</span>
                <span className="font-medium">{venue.name}</span>
                <span className="block text-stone-400 text-xs mt-0.5">{venue.neighborhood}</span>
              </td>
              ...
              <td className="px-4 py-3 text-right font-medium text-brand-700">{venue.dayPassPrice}</td>
            </tr>
          ))}
        </tbody>
      </table>

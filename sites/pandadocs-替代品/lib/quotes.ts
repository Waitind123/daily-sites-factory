...
function quoteNumber(): string {
  ...
  return `SP-${new Date().getFullYear()}-${n}`;
}
...
Document #${quote.quoteNumber} · ${today}
...
E-SIGNATURE
By signing electronically below, both parties agree this agreement is legally binding.
...
Invoice #${quote.quoteNumber.replace("SP", "INV")}
Date: ${today}
Due: ${dueDate}
...
export function collectSignature(
  quoteId: string
): { signedAt: string; signedBy: string } | null {
  const quote = quotes.get(quoteId);
  if (!quote || !quote.contractText) return null;

  const signedAt = new Date().toISOString();
  const signedBy = quote.clientName;

  quote.signedAt = signedAt;
  quote.signedBy = signedBy;
  quote.status = "paid";
  quotes.set(quoteId, quote);

  return { signedAt, signedBy };
}

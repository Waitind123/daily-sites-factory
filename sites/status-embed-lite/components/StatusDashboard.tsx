...
export function StatusDashboard({ locale }: { locale: Locale }) {
  ...
  function getEmbedCode(slug: string) {
    ...
    return `<iframe src="${origin}/embed/${slug}" width="180" height="28" frameborder="0"></iframe>`;
  }
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      {created && (
        <div className="rounded-xl border border-emerald-600/40 bg-emerald-950/20 p-6 mb-8">
          ...
          <p className="mt-3 text-sm font-medium text-foreground">{t.embedCode}</p>
          <code className="mt-1 block rounded-lg bg-background border border-border px-4 py-3 text-xs text-brand-400 break-all font-mono">
            {getEmbedCode(created.slug)}
          </code>
          <button
            type="button"
            onClick={() => copyEmbedCode(created.slug)}
            className="mt-3 rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-surface"
          >
            {copied ? t.copied : t.copyCode}
          </button>
          <Link
            href={`/s/${created.slug}`}
            className="mt-4 block text-sm font-medium text-brand-500 hover:text-brand-400"
          >
            {t.openPage}
          </Link>
        </div>
      ...
      {selectedPage && (
        <section className="rounded-xl border border-border bg-surface p-6 mb-8">
          <h2 className="font-semibold text-lg mb-2">{t.embedCode}</h2>
          <p className="text-sm text-muted mb-3">{t.createdHint}</p>
          <code className="block rounded-lg bg-background border border-border px-4 py-3 text-xs text-brand-400 break-all font-mono">
            {getEmbedCode(selectedPage.slug)}
          </code>
          <button
            type="button"
            onClick={() => copyEmbedCode(selectedPage.slug)}
            className="mt-3 rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-surface"
          >
            {copied ? t.copied : t.copyCode}
          </button>
          <div className="mt-6 pt-6 border-t border-border">
            <h3 className="font-medium text-foreground mb-1">{t.autoDetect}</h3>
            <p className="text-sm text-muted mb-3">{t.autoDetectDesc}</p>
            <input
              type="url"
              placeholder={t.pingUrlPlaceholder}
              className="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-brand-600"
              readOnly
              defaultValue=""
            />
          </div>

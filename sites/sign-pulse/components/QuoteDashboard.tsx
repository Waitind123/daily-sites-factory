...
export function QuoteDashboard({ locale }: { locale: Locale }) {
  ...
  async function handleSign(quoteId: string) {
    setGenerating(`sign-${quoteId}`);
    setError("");
    setShowPaywall(false);

    try {
      const res = await fetch(`/api/quotes/${quoteId}/sign`, { method: "POST" });
      const data = await res.json();
      ...
      if (created?.id === quoteId) {
        setCreated((c) =>
          c
            ? { ...c, signedAt: data.signedAt, signedBy: data.signedBy, status: "paid" }
            : c
        );
      }
      if (data.trial) setTrial(data.trial);
    } catch (err) {
      setError(err instanceof Error ? err.message : getApiErrorMessage("SIGN_FAILED", locale));
    } finally {
      setGenerating(null);
    }
  }
      ...
      {created && (
        <div className="rounded-xl border border-emerald-600/40 bg-emerald-950/20 p-6 mb-8">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            {created.contractText && !created.signedAt && (
              <button
                type="button"
                onClick={() => handleSign(created.id)}
                disabled={generating === `sign-${created.id}`}
                className="rounded-lg border border-emerald-600/50 px-6 py-2.5 font-semibold text-emerald-400 hover:bg-emerald-950/30 disabled:opacity-50"
              >
                {generating === `sign-${created.id}` ? t.signing : t.genSign}
              </button>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {quotes.map((q) => (
              <div key={q.id} className="rounded-xl border border-border bg-surface p-5">
                ...
                {q.signedAt ? (
                  <div className="mt-4 rounded-lg bg-emerald-950/20 border border-emerald-600/30 p-4">
                    <p className="text-xs font-medium text-emerald-400 mb-1">{t.signedLabel}</p>
                    <p className="text-sm text-muted">
                      {t.signedAt}: {q.signedBy} · {new Date(q.signedAt).toLocaleDateString()}
                    </p>
                  </div>
                ) : q.contractText ? (
                  <button
                    ...
                    disabled={generating === `sign-${q.id}`}
                    className="mt-2 text-sm font-medium text-emerald-400 hover:text-emerald-300 disabled:opacity-50"
                  >
                    {generating === `sign-${q.id}` ? t.signing : t.genSign}
                  </button>
                ) : null}
              </div>
            ))}

...
function heatmapClass(status: string) {
  if (status === "hot") return "bg-red-500/20 text-red-400";
  if (status === "warm") return "bg-amber-500/20 text-amber-400";
      ...
      {result && (
        <div className="mb-8 space-y-6">
          ...
          <div className="rounded-xl border border-border bg-surface p-6">
            <h3 className="font-semibold text-foreground mb-4">{t.dropoutTitle}</h3>
            <div className="space-y-3">
              {result.dropoutZones.map((zone) => (
                <div key={zone.step} className="rounded-lg border border-border bg-background p-4">
                  <div className="flex justify-between items-start gap-2">
                    <p className="font-medium text-foreground text-sm">{zone.step}</p>
                    <span className="text-xs px-2 py-0.5 rounded-full shrink-0 bg-red-500/20 text-red-400">
                      {t.dropoutLabel}: {zone.dropoffPct}%
                    </span>
                  </div>
                  <p className="text-sm text-muted mt-2">{zone.issue}</p>
                  <p className="text-sm text-brand-500 mt-2">{zone.fix}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-surface p-6">
            <h3 className="font-semibold text-foreground mb-4">{t.heatmapTitle}</h3>
            <div className="space-y-3">
              {result.heatmapZones.map((zone) => (
                <div key={zone.zone} className="rounded-lg border border-border bg-background p-4">
                  <div className="flex justify-between items-start gap-2">
                    <p className="font-medium text-foreground text-sm">{zone.zone}</p>
                    <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 ${heatmapClass(zone.status)}`}>
                      {t.attentionLabel}: {zone.attentionPct}%
                    </span>
                  </div>
                  <p className="text-sm text-muted mt-2">{zone.insight}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-surface p-6">
            ...
            <div className="space-y-3">
              {result.copyImprovements.map((rewrite) => (

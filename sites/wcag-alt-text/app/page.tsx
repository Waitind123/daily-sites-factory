      ...
      <section className="bg-surface border-y border-border py-12">
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <div>
              <h2 className="text-xl font-bold mb-4">{c.auditTitle}</h2>
              <div className="space-y-3">
                {c.auditStats.map((s) => (
                  ...
                  >
                    ...
                    <span className="text-lg font-bold text-brand-500">{s.value}</span>
                  </div>
                ))}
              </div>
            </div>
            <div>
              <h2 className="text-xl font-bold mb-4">{c.sampleTitle}</h2>
              <div className="space-y-2">
                {c.sampleItems.map((item) => (
                  <div
                    key={item.id}
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-brand-500 font-medium">
                        {c.scoreLabel(item.score)}
                      </span>
                    </div>
                    <p className="text-xs text-muted truncate">{item.alt}</p>
                  </div>
                ))}
              </div>
            </div>

...
import { WEEKLY_TARGET } from "@/lib/copy";
...
export function HabitTracker({
  ...
  useEffect(() => {
    loadTrial();
    const saved = localStorage.getItem("guilt_free_habits_state");
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (habits.length) {
      localStorage.setItem("guilt_free_habits_state", JSON.stringify(habits));
    }
  }, [habits]);
      ...
      prev.map((h) =>
        h.id === habitId
          ? {
              ...h,
              doneToday: true,
              weekDone: h.doneToday ? h.weekDone : Math.min(h.weekDone + 1, 7),
            }
          : h
      )
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <p className="text-xs text-muted">{t.weeklyGoalShort}</p>
          <p className="text-lg font-bold text-brand-500">{WEEKLY_TARGET}/7</p>
        </div>
      </div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted">{t.weeklyLabel(habit.weekDone)}</p>
                <p
                  className={`text-xs mt-0.5 ${
                    habit.weekDone >= WEEKLY_TARGET ? "text-green-600" : "text-muted"
                  }`}
                >
                  {t.onTrack(habit.weekDone)}
                </p>
              </div>
            </div>
            <button
